...
export async function captureFigmaSnapshots(): Promise<FnResponse> {
  ...
  for (const { quarter, periodStart, periodEnd, final } of periods) {
  for (const { key } of fileKeys) {
    ...
    if (!ok || !payload) {
      ...
      results.push({ fileKey: key, quarter, ok: false, status, error: payload?.error });
      continue;
    }
    if (payload.analyticsAvailable === false) {
      results.push({ fileKey: key, quarter, ok: false, error: payload.analyticsNote });
      continue;
    }
    ...
    try {
      ...
      results.push({ fileKey: key, quarter, final, ok: true, inserts: row.inserts, detaches: row.detaches });
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
      console.error(`Snapshot upsert failed for ${key}: ${message}`);
      results.push({ fileKey: key, quarter, ok: false, error: message });
    }
  }
  }
